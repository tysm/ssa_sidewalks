...
class IoU:
    ...
    def evaluate(self, predictions, masks, device):
        with torch.no_grad():
            # create one-hot encoding of masks for each class
            masks_one_hot = torch.zeros_like(predictions).to(device=device)
            masks_one_hot.scatter_(1, masks.unsqueeze(dim=1), 1)

            # create one-hot encoding of predicted labels for each class
            predicted_one_hot = torch.zeros_like(predictions).to(device=device)
            predicted_one_hot.scatter_(1, torch.argmax(predictions, dim=1, keepdim=True), 1)

            # compute intersection and union for all classes
            intersection = (predicted_one_hot * masks_one_hot).sum(dim=(0, 2, 3))
            union = predicted_one_hot.sum(dim=(0, 2, 3)) + masks_one_hot.sum(dim=(0, 2, 3)) - intersection
